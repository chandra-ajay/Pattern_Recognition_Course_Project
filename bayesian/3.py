...
cv = 1.5*np.identity(32*32)
def disc(x_test):
	x_test_trans = x_test.transpose()
	tot_val = list()
	for i in range(0,c): 	#Discriminant Function for an identity covariance matrix
		mean_vector_trans = mv_list[i].transpose()
		cv_inv = np.linalg.inv(cv)
		
		val_1 = np.matmul(mean_vector_trans, x_test)
		
		val_2 = np.matmul(mean_vector_trans, mv_list[i])
		val_2 = -0.5*val_2
